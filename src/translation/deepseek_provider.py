...
class DeepSeekProvider(TranslationProvider):
    """DeepSeek 翻译后端，通过 OpenAI 兼容 API 调用 DeepSeek Chat。

    DeepSeek API 与 OpenAI chat/completions 格式兼容，
    使用 httpx + tenacity 进行 3 次指数退避重试。
    """

    def __init__(self, config: TranslationConfig) -> None:
        super().__init__(config)
        if not self.config.model or self.config.model == "glm-4-flash":
    ...
    def translate(
        ...
        """逐段翻译字幕，通过 httpx 调用 DeepSeek API，tenacity 3 次重试。

        Args:
            segments: 待翻译的字幕片段列表。
            progress_callback: 翻译进度回调。

        Returns:
            翻译完成后的字幕片段列表。

        Raises:
            PipelineError: API 调用失败时抛出。
        """
