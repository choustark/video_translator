...
from src.translation.deepseek_provider import DeepSeekProvider


def _make_config(**overrides) -> TranslationConfig:
    defaults = {"engine": "deepseek", "api_key": "test-api-key"}
...
def _make_provider(config: TranslationConfig, mock_client: MagicMock) -> DeepSeekProvider:
    provider = DeepSeekProvider(config)
    provider._client = mock_client
    return provider


class TestDeepSeekProviderTranslate:
    ...
    def test_sends_correct_api_request(self) -> None:
        mock_client = MagicMock()
        mock_client.post.return_value = _mock_response()
        provider = _make_provider(_make_config(api_key="sk-deepseek-key"), mock_client)
        provider.translate(_make_segments("Hello world"))
        call_kwargs = mock_client.post.call_args
        assert call_kwargs.kwargs["headers"]["Authorization"] == "Bearer sk-deepseek-key"
    ...
    def test_default_model_overrides_glm_default(self) -> None:
        mock_client = MagicMock()
        mock_client.post.return_value = _mock_response()
        provider = _make_provider(_make_config(model="glm-4-flash"), mock_client)
        provider.translate(_make_segments("Hello"))
        body = mock_client.post.call_args.kwargs["json"]
        assert body["model"] == "deepseek-chat"

    def test_custom_model_preserved(self) -> None:
        mock_client = MagicMock()
        mock_client.post.return_value = _mock_response()
        provider = _make_provider(_make_config(model="deepseek-reasoner"), mock_client)
        provider.translate(_make_segments("Hello"))
        body = mock_client.post.call_args.kwargs["json"]
...
class TestDeepSeekProviderRetryProgress:
...
class TestDeepSeekProviderSecurity:
    def test_api_key_not_in_logs(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        mock_client = MagicMock()
        mock_client.post.return_value = resp
        provider = _make_provider(_make_config(api_key="sk-super-secret"), mock_client)
